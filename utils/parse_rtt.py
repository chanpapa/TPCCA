            ...
            for delay, delay_var in delay_list:
                ...
                for iter_num in range(iteration):
                    ...
                    plt.savefig(os.path.join(
                        fig_folder,
                        f'uprttdiff-{link_trace}-{packet_buffer}-{delay}-{delay_var}-{iter_num}-longtermtput.png'
                    ),
                                bbox_inches='tight')
                    plt.close()
                    plt.figure(figsize=(12, 5))
...
#plot rtt in long term
                    for ccp_alg in ccp_algs:
                        ...
                        for line in lines:
                            ...
                            if len(time_wnd)==0:    
                                time_wnd=np.append(time_wnd,time-12)
                    ...
                    plt.title(f'uprttdiff-{link_trace}-{delay}-{delay_var}-longtermRTT')
                    plt.xlabel('Time (s)', fontsize='12')
                    plt.ylabel('RTT (ms)', fontsize='12')
                    plt.legend()
                    plt.savefig(os.path.join(
                        fig_folder,
                        f'uprttdiff-{link_trace}-{packet_buffer}-{delay}-{delay_var}-{iter_num}-longtermRTT.png'
                    ),
                                bbox_inches='tight')
                    plt.close()
                    plt.figure(figsize=(12, 5))
