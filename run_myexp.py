...
from utils.traceloader import TraceLoader
...
traceloader = TraceLoader(trace_list, trace_folder, link_trace_save, raw_data_save, trace_info_save)
traceloader.load()
    ...
    for packet_buffer in packet_buffer_list:
        for link_trace in trace_info:
            for delay, delay_var in delay_list:
                delay = delay
                delay_var = delay_var
                with open(f'{delay}-{delay_var}.txt','w') as f1:
                    f1.write(str(delay)+'\n')
                    f1.write(str(delay_var)+'\n')
                    f1.write(str(delay_var)+'\n')
                    f1.write(str(delay))
                f1.close()    
